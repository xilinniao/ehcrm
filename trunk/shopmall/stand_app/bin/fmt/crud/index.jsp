...
<title><%=projName %></title>
	...
	$(document).ready(function(){
		...
		mainTable = $('#id_mainTable').dataTable({
			...
			"sAjaxSource": "<%=path%>/${url}?method=list",
  ...
  <body class="input">
  	<div class="inputBar">
		<h1><span class="icon">&nbsp;</span>${moduleNameChn}</h1>
	</div>
	
	<div class="operateBar">
		<input type="button" class="addButton" onclick="location.href='${url}?method=add'" value="新增${moduleNameChn}">
	</div>
			...
			<table class="display stylized" id="id_mainTable">
				<thead class="table-header">
					<tr>
						...
						<#list attr as x><th nowrap><#if x.remark??>${x.remark}</#if></th></#list>
					</tr>
				</thead>							
			</table>
		</div>
