...
(function($, window, document) {
	...
	_oExt.oPagination = {
	     ...
	     *           node:nPaging - the DIV which contains this pagination control
			...
			"fnUpdate": function ( oSettings, fnCallbackDraw )
			{
				...
				var iPages = Math.ceil((oSettings.fnRecordsDisplay()) / oSettings._pageSize);
				var iCurrentPage = Math.ceil(oSettings._dataStart / oSettings._pageSize) + 1;
				var sList = "";
				...
				var fnClick = function() {
					...
					oSettings._dataStart = iTarget * oSettings._pageSize;
					fnCallbackDraw( oSettings );
					return false;
				};
				...
				for ( i=0, iLen=an.length ; i<iLen ; i++ )
				{
					...
					if ( iPages === 0 || iCurrentPage == iPages || oSettings._pageSize == -1 )
					{
	...
	$.fn.dataTable = function( oInit )
	{
		...
		function classSettings ()
		{
			...
			{
				if ( this.oFeatures.bServerSide ) {
					if ( this.oFeatures.bPaginate === false || this._pageSize == -1 ) {
						return this._dataStart+this.aiDisplay.length;
					} else {
						return Math.min( this._dataStart+this._pageSize, 
			 ...
			 */
			this.aLengthMenu = [ 1,10, 25, 50, 100 ];
			...
			/*
			 * Variable: _pageSize, _dataStart, _iDisplayEnd
		...
		this.fnDeleteRow = function( mTarget, fnCallBack, bRedraw )
		{
			...
			if ( oSettings._dataStart >= oSettings.aiDisplay.length )
			{
				oSettings._dataStart -= oSettings._pageSize;
				if ( oSettings._dataStart < 0 )
				{
					oSettings._dataStart = 0;
				}
			}
		...
		function _fnInitalise ( oSettings )
		{
			...
			if ( oSettings.sAjaxSource !== null && !oSettings.oFeatures.bServerSide )
			{
				...
				oSettings.fnServerData.call( oSettings.oInstance, oSettings.sAjaxSource, [], function(json) {
					 ...
					 */
					oSettings.iInitDisplayStart = oSettings._dataStart;
		...
		function _fnDraw( oSettings )
		{
			...
			if ( typeof oSettings.iInitDisplayStart != 'undefined' && oSettings.iInitDisplayStart != -1 )
			{
				if ( oSettings.oFeatures.bServerSide )
				{
					oSettings._dataStart = oSettings.iInitDisplayStart;
				}
				else
				{
					oSettings._dataStart = (oSettings.iInitDisplayStart >= oSettings.fnRecordsDisplay()) ?
			...
			{
				var iStart = oSettings._dataStart;
			...
			if ( typeof oSettings.fnHeaderCallback == 'function' )
			{
				oSettings.fnHeaderCallback.call( oSettings.oInstance, $('>tr', oSettings.nTHead)[0], 
					_fnGetDataMaster( oSettings ), oSettings._dataStart, oSettings.fnDisplayEnd(),
					oSettings.aiDisplay );
			}
		...
		function _fnAjaxUpdate( oSettings )
		{
			if ( oSettings.bAjaxDataGet )
			{
				...
				aoData.push( { "name": "dataStart",  "value": oSettings._dataStart } );
				aoData.push( { "name": "pageSize", "value": oSettings.oFeatures.bPaginate !== false ?
					oSettings._pageSize : -1 } );
		...
		function _fnSort ( oSettings, bApplyClasses )
		{
			...
			{
				oSettings.aiDisplay = oSettings.aiDisplayMaster.slice();
				oSettings._dataStart = 0; /* reset display back to page 0 */
				_fnCalculateEnd( oSettings );
				_fnDraw( oSettings );
			}
		}
		...
		{
			var iOldStart = oSettings._dataStart;
			...
			{
				oSettings._dataStart = oSettings._pageSize>=0 ?
					oSettings._dataStart - oSettings._pageSize :
					0;
			...
			{
				if ( oSettings._pageSize >= 0 )
				{
					/* Make sure we are not over running the display array */
					if ( oSettings._dataStart + oSettings._pageSize < oSettings.fnRecordsDisplay() )
					{
						oSettings._dataStart += oSettings._pageSize;
					}
				}
			...
			else if ( sAction == "last" )
			{
				if ( oSettings._pageSize >= 0 )
				{
					var iPages = parseInt( (oSettings.fnRecordsDisplay()-1) / oSettings._pageSize, 10 ) + 1;
					oSettings._dataStart = (iPages-1) * oSettings._pageSize;
				}
				else
				{
					oSettings._dataStart = 0;
			...
			return iOldStart != oSettings._dataStart;
		}
		...
		function _fnUpdateInfo ( oSettings )
		{
			...
			var
				iStart = oSettings._dataStart+1, iEnd = oSettings.fnDisplayEnd(),
				iMax = oSettings.fnRecordsTotal(), iTotal = oSettings.fnRecordsDisplay(),
				sStart = oSettings.fnFormatNumber( iStart ), sEnd = oSettings.fnFormatNumber( iEnd ),
				sMax = oSettings.fnFormatNumber( iMax ), sTotal = oSettings.fnFormatNumber( iTotal ),
				sOut;
			
			/* When infinite scrolling, we are always starting at 1. _dataStart is used only
			 * internally
			 */
			if ( oSettings.oScroll.bInfinite )
			{
		...
		function _fnFeatureHtmlLength ( oSettings )
		{
			 ...
			 */
			$('select option[value="'+oSettings._pageSize+'"]',nLength).attr("selected",true);
			...
			$('select', nLength).change( function(e) {
				...
				/* Redraw the table */
				oSettings._pageSize = parseInt(iVal, 10);
				_fnCalculateEnd( oSettings );
				
				/* If we have space to show extra rows (backing up from the end point - then do so */
				if ( oSettings.fnDisplayEnd() == oSettings.fnRecordsDisplay() )
				{
					oSettings._dataStart = oSettings.fnDisplayEnd() - oSettings._pageSize;
					if ( oSettings._dataStart < 0 )
					{
						oSettings._dataStart = 0;
					}
		...
		function _fnNodeToDataIndex( s, n )
		{
			 ...
			 */
			for ( i=s._dataStart, iLen=s._iDisplayEnd ; i<iLen ; i++ )
			{
		...
		function _fnCalculateEnd( oSettings )
		{
			...
			else
			{
				 ...
				 * still to display
				 */
				if ( oSettings._dataStart + oSettings._pageSize > oSettings.aiDisplay.length ||
					   oSettings._pageSize == -1 )
				{
					oSettings._iDisplayEnd = oSettings.aiDisplay.length;
				}
				...
				{
					oSettings._iDisplayEnd = oSettings._dataStart + oSettings._pageSize;
				}
			}
		}
		...
		function _fnSaveState ( oSettings )
		{
			...
			sValue += '"iStart":'+ oSettings._dataStart+',';
			sValue += '"iEnd":'+ oSettings._iDisplayEnd+',';
			sValue += '"iLength":'+ oSettings._pageSize+',';
		...
		function _fnLoadState ( oSettings, oInit )
		{
			...
			if ( sData !== null && sData !== '' )
			{
				...
				oSettings._dataStart = oData.iStart;
				oSettings.iInitDisplayStart = oData.iStart;
				oSettings._iDisplayEnd = oData.iEnd;
				oSettings._pageSize = oData.iLength;
		...
		return this.each(function()
		{
			...
			if ( typeof oInit != 'undefined' && oInit !== null )
			{
				...
				_fnMap( oSettings, oInit, "pageSize", "_pageSize" );
				...
				if ( typeof oInit.dataStart != 'undefined' && 
				     typeof oSettings.iInitDisplayStart == 'undefined' )
				{
					/* Display start point, taking into account the save saving */
					oSettings.iInitDisplayStart = oInit.dataStart;
					oSettings._dataStart = oInit.dataStart;
				}
