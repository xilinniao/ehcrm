 ...
 if((node.childNodes[i].unParsed)||(node.childNodes[i].childsCount>0))
{
 ...
 dhtmlXTreeObject.prototype.getAllLeafChecked=function(){
 return this._getAllLeafChecked("","",1);
}
 ...
 for(var i=0;i<j;i++)
{
 ...
 dhtmlXTreeObject.prototype._getAllLeafChecked=function(htmlNode,list,mode){
 if(!htmlNode)htmlNode=this.htmlNode;

 if(htmlNode.checkstate==mode)
 if(!htmlNode.nocheckbox && this.hasChildren(htmlNode.id)==false){if(list)list+=this.dlmtr+htmlNode.id;else list=htmlNode.id;}
 var j=htmlNode.childsCount;
 for(var i=0;i<j;i++)
{
 list=this._getAllLeafChecked(htmlNode.childNodes[i],list,mode);
};
